...
<div id="ssp_wrap" class="wrapp">
	...
	<h1 id="ssp_title">SEO SIMPLE PACK <?=__( 'General settings', 'loos-ssp' )?></h1>
	<?php
		if ( $is_updated ) {
		echo '<div id="ssp_updated" class="updated notice is-dismissible">' .
		'<p><strong>' . __( 'Your settings have been saved.', 'loos-ssp' ) . '</strong></p>' .
		'<button type="button" class="notice-dismiss">' .
		'<span class="screen-reader-text">' . __( 'Hide this notification.', 'loos-ssp' ) . '</span>' .
		'</button>' .
		'</div>';
		}
	?>
	...
	<div id="poststuff">
		<form action="" method="post" id="ssp_form" accept-charset="UTF-8">
			...
			<button type="submit" class="button button-primary"><?=__( 'Save settings', 'loos-ssp' )?></button>
		</form>
	</div>
</div>
